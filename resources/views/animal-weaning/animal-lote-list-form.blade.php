...
@else
<form id="form-lote" action="{{ route('lote-weaning') }}" method="POST">
    @csrf
    <div class="row">
        <div class="col-4">
            <div class="mb-3">
...
<script>
    ...
    var formLocal = document.getElementById('form-lote')
    if(formLocal) {
        formLocal.addEventListener('submit', (event) => {
            event.preventDefault();
            document.getElementById('lote_id').value = document.getElementById('lote').value
